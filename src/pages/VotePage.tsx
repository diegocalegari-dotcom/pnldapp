...
import BookSelectionTable from '../components/BookSelectionTable';
...
const VotePage: React.FC = () => {
  ...
  const hasProjects = eixoBooks.projects && eixoBooks.projects.length > 0;

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!textbook1 || !textbook2 || (hasProjects && (!project1 || !project2))) {
      setMessage('Erro: Todas as opções (1ª e 2ª para cada categoria) devem ser selecionadas.');
      return;
    }
    ...
    try {
      ...
      await submitVote(loggedInTeacher.id, votePayload);
  ...
  const formIsComplete = textbook1 && textbook2 && (!hasProjects || (project1 && project2));
