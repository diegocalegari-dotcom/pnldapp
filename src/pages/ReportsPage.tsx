...
import { Eixo, Book, Teacher } from '../types';
...
const ConsolidatedReport: React.FC = () => {
  ...
  const reportData = useMemo(() => {
    const results = Object.values(Eixo).reduce((acc, eixo) => {
        acc[eixo] = { textbook1: {}, textbook2: {}, project1: {}, project2: {} };
        return acc;
    }, {} as Record<Eixo, any>);
    ...
    teachers.forEach(teacher => {
        if (teacher.vote) {
            ...
            if (!eixoResult) return;
            const { textbook1, textbook2, project1, project2 } = teacher.vote;
            if (textbook1) eixoResult.textbook1[textbook1] = (eixoResult.textbook1[textbook1] || 0) + 1;
            if (textbook2) eixoResult.textbook2[textbook2] = (eixoResult.textbook2[textbook2] || 0) + 1;
            if (project1) eixoResult.project1[project1] = (eixoResult.project1[project1] || 0) + 1;
            if (project2) eixoResult.project2[project2] = (eixoResult.project2[project2] || 0) + 1;
        }
    });

    const findWinner = (votes: Record<string, number>): { book: Book | null; votes: number } => {
        const sorted = Object.entries(votes).filter(([code]) => code).sort((a, b) => b[1] - a[1]);
    ...
    <>
      ...
      <div className="space-y-8">
        {reportData.map(({ eixo, textbook1, textbook2, project1, project2 }) => {
            const hasProjects = BOOK_DATA[eixo]?.projects.length > 0;
            // Não renderiza o card se não houver votos para o eixo
            if (textbook1.votes === 0 && textbook2.votes === 0 && project1.votes === 0 && project2.votes === 0) {
                return null;
            }
            return (
              <Card key={eixo} className="print-no-break print-card">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                  <ReportItem title="1ª Opção - Livro Didático / Coleção" book={textbook1.book} votes={textbook1.votes} />
                  <ReportItem title="2ª Opção - Livro Didático / Coleção" book={textbook2.book} votes={textbook2.votes} />
                  {hasProjects && <ReportItem title="1ª Opção - Projeto Integrador" book={project1.book} votes={project1.votes} />}
                  {hasProjects && <ReportItem title="2ª Opção - Projeto Integrador" book={project2.book} votes={project2.votes} />}
                </div>
              </Card>
            )
        })}
        ...
        <>
            ...
            <div className="space-y-6">
                {teachers.map(teacher => {
                    const hasProjects = BOOK_DATA[teacher.eixo]?.projects.length > 0;
                    return (
                        <Card key={teacher.id} className="print-no-break print-card">
                            ...
                            {teacher.vote ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <VoteDetail title="1ª Opção - Livro Didático / Coleção" book={allBooks.get(teacher.vote.textbook1)} />
                                    <VoteDetail title="2ª Opção - Livro Didático / Coleção" book={allBooks.get(teacher.vote.textbook2)} />
                                    {hasProjects && <VoteDetail title="1ª Opção - Projeto Integrador" book={allBooks.get(teacher.vote.project1)} />}
                                    {hasProjects && <VoteDetail title="2ª Opção - Projeto Integrador" book={allBooks.get(teacher.vote.project2)} />}
                                </div>
                            ) : (
